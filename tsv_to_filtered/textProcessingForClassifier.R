...
# 
# Functionality simplified from filterTweets.R:removeWhitelistedTerms. 
...
checkForWhitelistTerms = function(whitelistTerms, tweets, removeWLTerms=FALSE) {
    ...
    #whiteBoundaries3 = paste0("(?<![\\w@])", whitelistLower, "(e?s)?(?!\\w)")  
    if (sum(endsInPunct)) {
    ...
    if (removeWLTerms) {
        ...
        #unmatchedCol2 = sapply(unmatchedParts, function(x) { paste(unique(x), collapse=" ") })
...
# Modified from filterTweets:tokenizeText.
#
# (Other things we could try:
# -might want to preserve emoticons, non-latin chars
# -preserve [@#] punctuation
# -break up camel case w/in urls)
tokenizeTweet = function(tweets, 
    ...
    # Could add a flag to remove predefined stopwords.
...
# ignoreWhitelistTerms: (not currently used) If a character vector of whitelist terms, then make a pseudo-doc containing these terms,
#   to see what columns they map to in the DTM. Before returning, delete both the pseudo-doc and those columns.
# Note: could parallelize this by adding a flag to create_dtm (plus some setup of the "foreach" library).
tokenizeToDocTermMatrix = function(tweets, modelFileOutStem=NULL, ignoreWhitelistTerms=NULL, loadVectorizer=F) {
    
    if (!is.null(ignoreWhitelistTerms)) {
        pseudoTweet = paste(ignoreWhitelistTerms, collapse=" ")
        tweets = c(tweets, pseudoTweet)
    }
    
    # Packages that should make this easy in R: tm (better established), or text2vec (newer, faster)
    # -With tm, disadvantages cropped up: dependency "slam" (the sparse matrix representation tm uses) can't be installed on server; 
    # slam gives either warning or error when tm::removeSparseTerms is called, because it internally converts to a dense matrix
    # that exceeds size limit.
    # -With text2vec --> so fast! Definitely better!
    ...
    #  -If I do specialized tokenizing (of entities), it belongs in this function.
    #  -Docs say stemming should be called from inside here.
    #  -word2vec::word_tokenizer splits at and removes all non-word chars (but does no stemming). 
    #   Docs suggest checking package "tokenizers" and stringi and stringr's str_split_* for better ones.
    
    #iterator_tweets = itoken(tweets, tokenizer=word_tokenizer)  # preprocessor=tolower not needed
    ...
    useTfIdf = F
    if (useTfIdf) {
        model_tfidf = TfIdf$new()   # object that can "fit" a docTermMatrix, "transform" it, or both. (Fit = use counts from.)
        dtMatrix_tfidf = model_tfidf$fit_transform(dtMatrix)
        # note: if we wanted to use tfidf, would need to also save this "fit" variable.
    }
    
    # Things one can try varying:
    #  -tfidf weighting
    #  -turn off stemming
    #  -turn on stopword removal
    #  -adjust min/max word freqs to keep
    #  -other filters like tm offered
    
    
    if (!is.null(ignoreWhitelistTerms)) {
        colsForWhitelistTerms = which(dtMatrix[nrow(dtMatrix),] > 0)
        # remove pseudoTweet
        dtMatrix = dtMatrix[1:(nrow(dtMatrix)-1),]
        # remove whitelistCols
        dtMatrix = dtMatrix[, ! (1:ncol(dtMatrix) %in% colsForWhitelistTerms)]
    }
